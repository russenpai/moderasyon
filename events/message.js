...
module.exports = class {
  ...
  async run(message) {
    ...
    if (message.channel.id == "759566000378871828") {
    ...
    if (!owners.includes(message.author.id)) {
      ...
      if (blockArr.length == 9) {
        ...
        this.client.channels.cache.get("760547740354084874").send(`**${message.author.tag}** - ${message.author}(\`${message.author.id}\`) komut engeli yedi.Komut kullanım özeti:\n\`\`\`${blockArr.map(x => x.içerik).join("\n")}\nKullandığı komutlar: ${blockArr.map(x => x.komut).join(",")}\nKullandığı kanallar: ${blockArr.map(x => x.kanal).join(",")}\`\`\``)
