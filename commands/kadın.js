...
class Kadın extends Command {
    ...
    async run(message, args, level) {
        ...
        if (user.roles.cache.some(x => ["727881569330528347", "728204860671131661", "749380214668132362", "727881569330528347", "728204860671131661", "728204858288766986"].includes(x.id))) {
        ...
        await kayıtlar.findOne({ user: message.author.id }, async (err, res) => {
            ...
            if (!user.roles.cache.has("727881569330528347")) {
                ...
                await user.roles.add(["727881569330528347", "728204860671131661", "749380214668132362"])
                await user.roles.remove(["759257818867564564", "747553063082000434"])
                const emux = new Discord.MessageEmbed()
                    .setAuthor(message.author.tag, message.author.displayAvatarURL({ dynamic: true }))
                    .setDescription(`${user} kullanıcısı başarıyla kadın olarak kayıt edildi.`)
                    .setColor("RANDOM")
